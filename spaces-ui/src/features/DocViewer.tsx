...
import { useDocContent } from '@/hooks/useSpaces'
...
export function DocViewer({ slug, docPath }: DocViewerProps) {
  const { data: content, isLoading, error } = useDocContent(slug, docPath ?? '')

  if (!docPath) {
    return (
      <div className="flex items-center justify-center h-full py-20 text-stone">
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <Loader2 className="h-6 w-6 text-stone animate-spin" />
      </div>
    )
  }
  ...
  return (
    <div className="p-4">
      <div className="mb-4 pb-3 border-b border-border-custom">
        <p className="font-mono text-xs text-stone">{docPath}</p>
      </div>
